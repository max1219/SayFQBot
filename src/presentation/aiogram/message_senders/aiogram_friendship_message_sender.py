...
class AiogramFriendshipMessageSender(IFriendshipMessageSender):
    ...
    async def send_friendship_request(self, id_from: int, id_to: int, name_from: str) -> bool:
        try:
            await self._bot.send_message(chat_id=id_to, text=f'Вам приглашение в друзья от {name_from}')
            return True
        except AiogramError:
    ...
    async def send_friendship_accepted(self, id_accepted: int, id_requested: int, name_accepted: str) -> bool:
        try:
            await self._bot.send_message(chat_id=id_requested, text=f'Теперь ты друг с {name_accepted}')
            return True
        except AiogramError:
